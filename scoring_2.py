...
def weighted_interval_score_fast(
    observations, alphas, medians, q_dict, weights=None, percent=False, check_consistency=True
):
    """
    Compute weighted interval scores for an array of observations and a number of different predicted intervals.
    ...
    lower_flag = np.sum(lower_calibrations > 0, axis =0) > 0
    upper_calibrations = (
        np.clip(observations - upper_quantiles, a_min=0, a_max=None) * upper_q_alphas
    )
    upper_flag = np.sum(upper_calibrations > 0, axis =0) > 0
    calibrations = lower_calibrations + np.flip(upper_calibrations, axis=0)

    # accounting for code discrepancies?
    # sharpnesses = np.append(sharpnesses, np.array(0))
    sharpnesses = np.vstack((sharpnesses, np.zeros([1, np.shape(sharpnesses)[1]])))

    # calibrations = np.append(calibrations, abs(medians - observations))
    calibrations = np.vstack((calibrations, np.reshape((np.clip(medians - observations, a_min=0, a_max=None) + np.clip(observations - medians, a_min=0, a_max=None)),[1, np.shape(calibrations)[1]])))
    lower_flag_median = np.clip(medians - observations, a_min=0, a_max=None) > 0
    upper_flag_median = np.clip(observations - medians, a_min=0, a_max=None) > 0
    # lower_calibrations = np.vstack((lower_calibrations, np.reshape(abs(np.clip(medians - observations, a_min=0, a_max=None)), [1, np.shape(lower_calibrations)[1]])))
    # upper_calibrations = np.vstack((upper_calibrations, np.reshape(abs(np.clip(observations - medians, a_min=0, a_max=None)), [1, np.shape(upper_calibrations)[1]])))
    lower_flag_combined = np.clip(lower_flag.astype(int) + lower_flag_median.astype(int), a_min=0, a_max=1)
    upper_flag_combined = np.clip(upper_flag.astype(int) + upper_flag_median.astype(int), a_min=0, a_max=1)
    ...
    if percent:
        ...
        # lower_calibrations = lower_calibrations / np.abs(observations)
        # upper_calibrations = upper_calibrations / np.abs(observations)
    ...
    k = len(alphas)  # number of intervals
    sharpnesses_final = np.sum(sharpnesses_weighted, axis=0) / (k+1/2)
    calibrations_final = np.sum(calibrations_weighted, axis=0) / (k+1/2)
    lower_calibrations_final = calibrations_final * lower_flag_combined
    upper_calibrations_final = calibrations_final * upper_flag_combined
    totals_final = np.sum(totals_weighted, axis=0) / (k+1/2)
